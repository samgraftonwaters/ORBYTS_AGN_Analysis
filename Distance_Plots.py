...
for YEAR, colors in zip(year, c):
    ...
    #This reads the spreadsheet from the YEAR
    ...
    plt.errorbar(cen, distance, xerr=cen_err, yerr=distance_err, color=colors, fmt='o', markersize=10, label=YEAR)

plt.yscale('log')
plt.xlabel(r'Wavelength ($\AA$)', fontsize=20)
plt.ylabel(r'Distance (m)', fontsize = 25)
plt.title(AGN+': Distance vs Wavelength', fontsize=30, loc='center')
